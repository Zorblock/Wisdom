...
pub fn login(
    ...
    let client = http()?;
    report("Opening Microsoft sign-in …".into());
...
pub fn ensure_session(client_id: &str) -> Result<AuthState> {
    ...
    let refreshed = refresh_microsoft_session(client_id, &auth).context("Could not refresh Microsoft session")?;
    save_auth(&refreshed)?;
    Ok(refreshed)
}
...
fn poll_device_code(client: &Client, client_id: &str, device: &DeviceCode, cancelled: &AtomicBool) -> Result<OAuthToken> {
    ...
    for _ in 0..180 {
        ...
        match body["error"].as_str() {
            ...
            None => bail!("Microsoft returned {status}"),
        }
    }
    bail!("The Microsoft sign-in code expired")
}
